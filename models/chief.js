...
const postChief = async function (chief, cb) {
  ...
  pool.query(
    "INSERT INTO chiefs (name, surname, password, phone_number) VALUES ($1, $2, $3, $4) RETURNING chief_id",
    [chief.name, chief.surname, chief.password, chief.phone_number],
    (err, res) => {
...
function get(chief_id, cb) {
  pool.query(
    "SELECT name, chief_id, password FROM chiefs WHERE chief_id = $1",
    [chief_id],
    (err, res) => {
